    ...
    <div class="flex h-screen bg-gray-50">
      ...
      <div :class="[
        ...
        <div class="p-6 border-b border-gray-100">
          <div class="flex items-center justify-between">
            <router-link to="/" class="flex items-center space-x-3">
              ...
              <span v-if="!isCollapsed" class="text-xl font-bold text-gray-900">Lnaid</span>
            </router-link>
            <button 
              @click="toggleSidebar"
        ...
        <div class="p-6 border-b border-gray-100">
          <div class="flex items-center space-x-3">
            ...
            <div v-if="!isCollapsed" class="flex-1 min-w-0">
              <p class="text-lg font-semibold text-gray-900 truncate">{{ authStore.user?.name }}</p>
          ...
          <div v-for="item in navigationItems" :key="item.name">
            ...
            >
              ...
              <div v-if="item.badge && !isCollapsed" class="ml-auto">
                <span class="bg-coral-500 text-white text-xs px-2 py-1 rounded-full">{{ item.badge }}</span>
              </div>
            </router-link>
            ...
            <div v-else>
              ...
              >
                <component :is="item.icon" class="w-6 h-6 flex-shrink-0" />
                <span v-if="!isCollapsed" class="ml-3 font-medium flex-1 text-left">{{ item.name }}</span>
                <svg 
                  v-if="!isCollapsed"
                  :class="[
              ...
              <!-- Submenu -->
              <div 
                v-if="openSubmenus.includes(item.name) && !isCollapsed"
        ...
        <div class="p-4 border-t border-gray-100">
          <button
            @click="logout"
            class="w-full flex items-center px-4 py-3 rounded-xl text-gray-700 hover:bg-red-50 hover:text-red-600 transition-all duration-200"
          >
            <svg class="w-6 h-6 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"></path>
            </svg>
            <span v-if="!isCollapsed" class="ml-3 font-medium">Sign Out</span>
          </button>
        </div>
      </div>
            ...
            <div class="flex items-center space-x-4">
              ...
              <div class="relative">
                ...
                >
                  ...
                  <span v-if="notificationStore.unreadCount > 0" class="absolute -top-1 -right-1 bg-coral-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center">
                    {{ notificationStore.unreadCount }}
                  </span>
                </button>
  
                <!-- Notifications Dropdown -->
                <div v-if="showNotifications" class="absolute right-0 mt-2 w-80 bg-white rounded-xl shadow-xl z-50 border border-gray-100">
                  <div class="p-4 border-b border-gray-100">
                    <div class="flex justify-between items-center">
                      <h3 class="font-semibold text-gray-900">Notifications</h3>
                      <button @click="notificationStore.markAllAsRead()" class="text-sm text-coral-500 hover:text-coral-600">
                        Mark all read
                      </button>
                    </div>
                  </div>
              ...
              <div class="relative">
                ...
                >
                  <img :src="authStore.user?.avatar" :alt="authStore.user?.name" class="w-8 h-8 rounded-lg">
                ...
                <!-- User Dropdown -->
                <div v-if="showUserMenu" class="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-xl z-50 border border-gray-100">
                  <div class="py-2">
                    <router-link to="/sponsor/profile" class="block px-4 py-2 text-gray-700 hover:bg-gray-50">Profile</router-link>
                    <router-link to="/settings" class="block px-4 py-2 text-gray-700 hover:bg-gray-50">Settings</router-link>
                    <button @click="logout" class="block w-full text-left px-4 py-2 text-gray-700 hover:bg-gray-50">Sign out</button>
                  </div>
                </div>
              </div>
            </div>

  ...
  <StudentSidebar>
    <div class="min-h-screen bg-gray-50">
      ...
      <div class="bg-white border-b border-gray-200">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div class="flex items-center justify-between h-16">
            <div>
              <h1 class="text-2xl font-bold text-gray-900">Browse Opportunities</h1>
              <p class="text-sm text-gray-500">Discover funding opportunities that match your profile</p>
            </div>
            <div class="flex items-center space-x-4">
              <span class="text-sm text-gray-600">{{ filteredOffers.length }} opportunities available</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <div class="flex items-center justify-between">
              <div>
                ...
                <p class="text-3xl font-bold text-gray-900 mt-2">{{ allOffers.length }}</p>
              </div>
          ...
          <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <div class="flex items-center justify-between">
              <div>
                ...
                <p class="text-3xl font-bold text-gray-900 mt-2">{{ matchingOffers.length }}</p>
              </div>
          ...
          <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <div class="flex items-center justify-between">
              <div>
                ...
                <p class="text-3xl font-bold text-gray-900 mt-2">₦{{ totalFunding.toLocaleString() }}</p>
              </div>
          ...
          <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <div class="flex items-center justify-between">
              <div>
                ...
                <p class="text-3xl font-bold text-gray-900 mt-2">₦{{ averageAmount.toLocaleString() }}</p>
              </div>
        ...
        <!-- Offers Grid -->
        <div v-if="filteredOffers.length === 0" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-12 text-center">
          ...
          <p class="text-gray-600 mb-6">
            {{ showMatchesOnly ? "No opportunities match your profile. Try browsing all opportunities." : "No opportunities match your current filters." }}
          </p>
          <button
            ...
            <div class="p-6">
              <div class="flex items-start justify-between mb-4">
                <div class="flex-1">
                  <div class="flex items-center space-x-3 mb-2">
                    ...
                    <span v-if="isMatchingOffer(offer)" class="bg-green-100 text-green-700 text-xs px-2 py-1 rounded-full font-medium">
                      Perfect Match
                    </span>
                    <span 
                  ...
                  <div class="flex items-center space-x-2 mb-2">
                    ...
                    >
                    <span class="text-gray-600">{{ offer.sponsorName }}</span>
                  </div>
                  <p class="text-gray-700 line-clamp-2">{{ offer.description }}</p>
                </div>
                <div class="text-right ml-6">
                  <p class="text-2xl font-bold text-coral-600">₦{{ offer.amount.toLocaleString() }}</p>
              ...
              <div class="mb-4">
                <div class="flex items-center justify-between text-sm text-gray-600 mb-2">
                  <span>Recipients: {{ offer.currentRecipients }}/{{ offer.maxRecipients }}</span>
              ...
              <div class="flex flex-wrap gap-2 mb-4">
                <span v-if="offer.criteria.minGPA" class="bg-blue-100 text-blue-700 text-xs px-2 py-1 rounded-full">
                  Min GPA: {{ offer.criteria.minGPA }}
                </span>
                <span v-if="offer.criteria.gender !== 'any'" class="bg-purple-100 text-purple-700 text-xs px-2 py-1 rounded-full">
                  {{ offer.criteria.gender }} only
                </span>
                <span v-if="offer.criteria.financialNeed" class="bg-orange-100 text-orange-700 text-xs px-2 py-1 rounded-full">
...
const filteredOffers = computed(() => {
  ...
  if (filters.course) {
    filtered = filtered.filter(offer => 
      offer.criteria.courses?.includes(filters.course) || offer.criteria.courses?.length === 0
    )
  }

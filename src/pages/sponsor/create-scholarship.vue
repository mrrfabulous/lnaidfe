<template>
  <Layout>
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900">Create Scholarship Program</h1>
        <p class="text-gray-600 mt-2">Set up a comprehensive scholarship program with multi-step application process</p>
      </div>

      <form @submit.prevent="handleSubmit" class="bg-white rounded-lg shadow p-8 space-y-8">
        <!-- Basic Information -->
        <div class="space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label for="title" class="block text-sm font-medium text-gray-700 mb-2">Scholarship Title *</label>
              <input
                id="title"
                v-model="form.title"
                type="text"
            ...
            <div>
              <label for="type" class="block text-sm font-medium text-gray-700 mb-2">Scholarship Type *</label>
              <select
                id="type"
                v-model="form.type"
                required
              ...
              >
                ...
                <option value="partial_tuition">Partial Tuition Coverage</option>
            ...
            <div>
              <label for="amount" class="block text-sm font-medium text-gray-700 mb-2">Amount per Recipient (₦) *</label>
              <input
                id="amount"
            ...
            <div>
              <label for="numberOfAwards" class="block text-sm font-medium text-gray-700 mb-2">Number of Awards *</label>
              <input
                id="numberOfAwards"
        ...
        <div class="space-y-6">
          <h2 class="text-xl font-semibold text-gray-900 border-b pb-2">Eligibility Criteria</h2>
          
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <!-- Schools -->
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">Eligible Schools</label>
              <div class="space-y-2">
                <div class="flex">
                  <input
                    v-model="newSchool"
                ...
                <p class="text-xs text-gray-500">Leave empty to include all schools</p>
              </div>
            </div>

            <!-- Courses -->
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">Eligible Courses</label>
              <div class="space-y-2">
                <div class="flex">
                  <input
                    v-model="newCourse"
                ...
                <p class="text-xs text-gray-500">Leave empty to include all courses</p>
              </div>
            </div>
            ...
            <div>
              ...
              <div class="space-y-2">
                <div v-for="level in availableLevels" :key="level" class="flex items-center">
                  ...
                  >
                  <label :for="`level-${level}`" class="ml-2 block text-sm text-gray-900">
                    {{ level }}
                  </label>
                </div>
              </div>
            ...
            <!-- GPA Requirement -->
            <div>
              <label for="minGPA" class="block text-sm font-medium text-gray-700 mb-2">Minimum GPA Required</label>
              <input
                id="minGPA"
              ...
              <div class="space-y-2">
                <label class="inline-flex items-center">
                  <input
                    ...
                    class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded"
                  >
                  <span class="ml-2 text-sm text-gray-900">Financial need required</span>
                </label>
                <label class="inline-flex items-center">
                  <input
                    ...
                    class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded"
                  >
                  <span class="ml-2 text-sm text-gray-900">Academic excellence required</span>
                </label>
              </div>
            </div>
          </div>
        ...
        <!-- Application Process -->
        <div class="space-y-6">
          <h2 class="text-xl font-semibold text-gray-900 border-b pb-2">Application Process</h2>
          
          <div class="space-y-4">
            <div v-for="(step, index) in form.applicationProcess" :key="index" class="border border-gray-200 rounded-lg p-4">
              <div class="flex justify-between items-start mb-4">
                <h3 class="text-lg font-medium text-gray-900">Step {{ step.step }}: {{ step.title }}</h3>
                <button
                  type="button"
        ...
        <!-- Requirements -->
        <div class="space-y-6">
          <h2 class="text-xl font-semibold text-gray-900 border-b pb-2">Application Requirements</h2>
          
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Required Documents/Information</label>
            <div class="space-y-2">
              <div class="flex">
                <input
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
          <!-- Benefits -->
          <div class="space-y-4">
            <h2 class="text-xl font-semibold text-gray-900 border-b pb-2">Scholarship Benefits</h2>
          ...
          <!-- Obligations -->
          <div class="space-y-4">
            <h2 class="text-xl font-semibold text-gray-900 border-b pb-2">Recipient Obligations</h2>
        ...
        <div class="space-y-6 border-t pt-6">
          ...
          <div class="bg-gray-50 rounded-lg p-6">
            <div class="flex justify-between items-start mb-4">
              <div>
                <h3 class="text-lg font-semibold text-gray-900">{{ form.title || 'Scholarship Title' }}</h3>
                <p class="text-sm text-gray-600">by {{ authStore.user?.name || 'Your Name' }}</p>
              </div>
              <div class="text-right">
                <div class="text-2xl font-bold text-purple-600">₦{{ form.amount?.toLocaleString() || '0' }}</div>
                <div class="text-sm text-gray-500">{{ form.type || 'Scholarship Type' }}</div>
              </div>
            </div>
            <p class="text-gray-700 mb-4">{{ form.description || 'Scholarship description will appear here...' }}</p>
            ...
            <div class="grid grid-cols-2 gap-4 text-sm mb-4">
              ...
              <div>
                ...
                <span class="ml-2">{{ formatDate(form.deadline) || 'Not set' }}</span>
              </div>
              <div>
                <span class="font-medium text-gray-900">Min GPA:</span>
                <span class="ml-2">{{ form.criteria.minGPA || 'Not specified' }}</span>
              </div>
            </div>

            <div v-if="form.applicationProcess.length" class="mb-4">
              <h4 class="font-medium text-gray-900 mb-2">Application Process:</h4>
              <div class="flex flex-wrap gap-2">
                <span
            ...
            <div v-if="form.benefits.length" class="mb-4">
              ...
              <ul class="text-sm text-gray-700 list-disc list-inside">
                <li v-for="benefit in form.benefits.slice(0, 3)" :key="benefit">{{ benefit }}</li>
        ...
        <div class="border-t pt-6">
          <div class="flex justify-end space-x-4">
            <router-link to="/sponsor/dashboard" class="px-6 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50">
              Cancel
            </router-link>
            <button
              type="submit"
...
const isFormValid = computed(() => 
  form.title && form.description && form.type && form.amount && 
  form.duration && form.numberOfAwards && form.deadline
)

const addSchool = () => {
  if (newSchool.value.trim() && !form.criteria.schools.includes(newSchool.value.trim())) {
...
const addCourse = () => {
  if (newCourse.value.trim() && !form.criteria.courses.includes(newCourse.value.trim())) {
...
const addObligation = () => {
  if (newObligation.value.trim() && !form.obligations.includes(newObligation.value.trim())) {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const newScholarship = await scholarshipsStore.createScholarship(scholarshipData)
    
    notificationStore.addNotification({
      type: 'success',
      title: 'Scholarship created successfully!',
      message: 'Your scholarship program is now live and accepting applications.'
    })
    ...
    notificationStore.addNotification({
      ...
      message: 'An error occurred while creating your scholarship. Please try again.'
    })
  } finally {
    loading.value = false
  }
